...
var JwtMultiSelect=React.createClass({
  	...
  	render: function(){
  		...
  		if(this.props.onClick){
   		...
   		return <div className="multiselect" style={{minWidth:this.props.hwidth}}>   			
   			<div ref="header" className={cssClass('header', {'has-error': this.props.hasError})} style={{width:this.props.hwidth}} onClick={this.onHeaderClick}><span ref="selectedText">0 selected</span>
   			...
   			<div ref="mscontent" className="ms-content" style={{width:this.props.width}}>
   				...
   				me.state.data.map(function(itemData, index){
   					return <JwtSelectItem key={index} data={itemData} index={index} displayField={me.props.displayField} render={me.props.render} checkItem={me.checkItem} />
   				})
   			}</div>
   			{submit}
   			</div>
